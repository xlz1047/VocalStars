"""Training script for the breath detection model.

Trains a BreathModel on pre-extracted NPZ features.  Breath labels are
derived at load time from VAD features — no re-extraction needed.
...
from ml_new.models.breath_model import BreathModel
...
def focal_bce(
    ...
    """Focal BCE with pos_weight to compensate for breath frame rarity (~3-8%)."""
    eps = 1e-7
...
def train(
    ...
    train_ds = FeatureDataset(manifest, seq_len=seq_len, split="train", task="breath")
    val_ds = FeatureDataset(manifest, seq_len=seq_len, split="val", task="breath")
    log.info("Train: %d clips | Val: %d clips", len(train_ds), len(val_ds))

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False, num_workers=0)

    model = BreathModel(hidden_size=hidden_size, num_layers=num_layers, dropout=dropout).to(device)
    log.info("BreathModel parameters: %d", model.param_count())
    ...
    with open(log_path, "w", newline="") as fh:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch in train_loader:
                ...
                target = batch["breath"].to(device)

                pred, _ = model(vad_feats)
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train breath detection model.")
    p.add_argument("--manifest", type=Path, default=Path("ml_new/data/extracted_pyin/manifest.csv"))
    p.add_argument("--output-dir", type=Path, default=Path("ml_new/checkpoints/breath"))
